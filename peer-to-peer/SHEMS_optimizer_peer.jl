function SHEMS_optimizer_peer(sh, hp, fh, hw, b, m)
    ...
    @variables(model, begin
        X[1:m.h_predict, 1:length(flows), 1:sh.n_peers] >= 0;    #1:PV_DE, 2:B_DE, 3:GR_DE, 4:PV_B, 5:PV_GR, 6:PV_HP, 7:GR_HP, 8:B_HP, 9:HP_FH, 10:HP_HW, 11:PV_PM, 12:B_PM, 13:PM_DE, 14:PM_B, 15:PM_HP
        SOC_b[1:m.h_predict+1, 1:sh.n_peers]  >= 0;
        T_fh[1:m.h_predict+1, 1:sh.n_peers] >= 0; V_hw[1:m.h_predict+1, 1:sh.n_peers] >= 0;
        Mod_fh[1:m.h_predict, 1:sh.n_peers] >= 0; Mod_hw[1:m.h_predict, 1:sh.n_peers]   >= 0;
        T_fh_plus[1:m.h_predict, 1:sh.n_peers] >= 0; T_fh_minus[1:m.h_predict, 1:sh.n_peers] >= 0;
        V_hw_plus[1:m.h_predict, 1:sh.n_peers] >= 0; V_hw_minus[1:m.h_predict, 1:sh.n_peers] >= 0;
        HP_switch[1:m.h_predict, 1:sh.n_peers], Bin;
    ...
    @objective(model, Max, sum( sum( (sh.p_sell *X[h,5,n])+
                    sum(sh.p_peer *X[h,i,n] for i=[11,12])-
                    sum(sh.p_buy *X[h,i,n] for i=[3,7])-
                    sum(sh.p_peer *X[h,i,n] for i=13:15)-
                    (sh.costfactor *(V_hw_plus[h,n] +V_hw_minus[h,n] +T_fh_plus[h,n] +T_fh_minus[h,n]))  for h=1:m.h_predict) for n=1:sh.n_peers));

    # Electricity demand, generation, market clearing___________________________________
    @constraints(model, begin
        [h=1:m.h_predict, n=1:sh.n_peers],     sum(X[h,i,n] for i=[1,2,3,13]) >= d_e[h];                   # fulfill energy demand (== for max. self-consmption)
        [h=1:m.h_predict, n=1:sh.n_peers],     sum(X[h,i,n] for i=[1,4,5,6,11]) <= g_e[h];                 # restricted by PV generation
        [h=1:m.h_predict],                     sum(sum(X[h,i,n] for i=[11,12]) for n=1:sh.n_peers) ==
                                                                    sum(sum(X[h,i,n] for i=13:15) for n=1:sh.n_peers);  # market clearing peer market
    end)
    # Battery__________________________________________________________________________
    @constraints(model, begin
        [h=1:m.h_predict-1, n=1:sh.n_peers],   SOC_b[h+1,n] == ((1 -b.loss) *SOC_b[h,n])+
                                                    (b.eta *sum(X[h,i,n] for i=[4,14]))-
                                                    sum((1.0/(b.eta)) *X[h,i,n] for i=[2,8,12]);    # State of Charge, loss for unique solutions
        [h=1:m.h_predict, n=1:sh.n_peers],     b.soc_min <= SOC_b[h,n] <= b.soc_max;                # Limits Battery usable capacity
        [h=1:m.h_predict, n=1:sh.n_peers],     sum(X[h,i,n] for i=[2,4,8,12,14]) <= b.rate_max;     # limit discharging/charging to nominal power (never at same time)
    end)
    # Heat pump_______________________________________________________________________________
    @constraints(model, begin
        [h=1:m.h_predict, n=1:sh.n_peers],     sum(X[h,i,n] for i=[9,10]) <=
                                                    sum(X[h,i,n] for i=[6,7,8,15])                  # level power heat pump=and out (== for max. self-consmption)
        [h=1:m.h_predict, n=1:sh.n_peers],     X[h,9,n] == Mod_fh[h,n] *hp.rate_max;                # heating energy FH max cap
        [h=1:m.h_predict, n=1:sh.n_peers],     X[h,10,n] == Mod_hw[h,n] *hp.rate_max;               # heating energy HW max cap
        #[h=1:m.h_predict, n=1:sh.n_peers],    Mod_hw[h,n] + Mod_fh[h,n] >= 0.1;                    # minimum level on?
        [h=1:m.h_predict, n=1:sh.n_peers],     Mod_fh[h,n] <= 1 -HP_switch[h,n];                    # switch FH or HW
        [h=1:m.h_predict, n=1:sh.n_peers],     Mod_hw[h,n] <= HP_switch[h,n];                       # switch HW or FH
    end)
    # Floor_heating__________________________________________________________________________________________
    @constraints(model, begin
        [h=1:m.h_predict-1, n=1:sh.n_peers],   T_fh[h+1,n] == T_fh[h,n]+
                                                    (60*60)/(p_concr *fh.volume *c_concr)*
                                                    ((cop_fh[h] *X[h,9,n]) -d_fh[h]-
                                                    ((1-Hot[h]) *fh.loss) +(Hot[h] *fh.loss));    # SoC floor heating (temperature)
        [h=1:m.h_predict, n=1:sh.n_peers],     T_fh[h,n] -((1 -Hot[h])*m.big) <= t_outside[h];          # force hot binary on if hotter outside than inside
        [h=1:m.h_predict, n=1:sh.n_peers],     t_outside[h] -(Hot[h]*m.big) <= T_fh[h,n];
        [h=1:m.h_predict, n=1:sh.n_peers],     T_fh[h,n] <= fh.soc_max +T_fh_plus[h,n];                # Limits temperature FH max
        [h=1:m.h_predict, n=1:sh.n_peers],     fh.soc_min -T_fh_minus[h,n] <= T_fh[h,n];               # Limits temperature FH min
    end)
    #Hot water_____________________________________________________________________________________________
    @constraints(model, begin
        [h=1:m.h_predict-1, n=1:sh.n_peers],   V_hw[h+1,n] == V_hw[h,n]+
                                                    (60*60)/((p_water *hw.t_supply *c_water)/1000)*
                                                    ((cop_hw[h] *X[h,10,n]) -d_hw[h] -hw.loss);        # SoC hot water (volume)
        [h=1:m.h_predict, n=1:sh.n_peers],     V_hw[h,n] <= hw.soc_max +V_hw_plus[h,n];                # Limits volume HW max
        [h=1:m.h_predict, n=1:sh.n_peers],     hw.soc_min - V_hw_minus[h,n] <= V_hw[h,n];              # Limits volume HW min
    ...
    # collect returns
    results = Array{Float64}(undef,(m.h_control*sh.n_peers, 11+15+3));    # optimization variables + flows + month + day + hour
    for n=1:sh.n_peers

        profits = (sh.p_sell .*JuMP.value.(X[1:m.h_control,5,n]))-
                    sum(sh.p_buy .*JuMP.value.(X[1:m.h_control,i,n]) for i=[3,7])+
                    sum(sh.p_peer *JuMP.value.(X[1:m.h_control,i,n]) for i=[11,12])-
                    sum(sh.p_peer *JuMP.value.(X[1:m.h_control,i,n]) for i=13:15);
        ...
        results[((n-1)*m.h_control)+1:(n*m.h_control),:] =
            ...
            profits, cop_fh[1:m.h_control], cop_hw[1:m.h_control], (ones(m.h_control)*n),
